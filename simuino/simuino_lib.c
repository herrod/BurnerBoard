...
//====================================
{
  int x, y, i,j,k=0,m=0,prevL=1;
  char *buf = (char *)malloc(MAX_STEP * 240);
  char filler[1024];
  int r, g, b;
	char pix[2] = {35, 0};
  ...
  for(i=1;i<=currentStep;i++) {
    if(strlen(serialM[i]) > 0)
    {
	  if(prevL == 1) //New Line
	    {
	      m++;
	      strcpy(&buf[m * 240],serialM[i]);
	    }
	  else
	    strcat(&buf[m * 240],serialM[i]);
  ...
  for(i=1;i<ser_h-1;i++)
    {
      ...
      if(k>0)
	{
	  fill(ser_w-strlen(&buf[k * 240]),filler,' ');
	  wprintw(ser,"%s%s",&buf[k * 240],filler);
	}
      else
  ...
  for(i=1;i<=currentStep;i++) {
        wmove(ser, 1, 1);
//        wprintw(ser, "Searching step %d = %d, last = %d        ", i, vScreenSteps[i], m);
        show(ser);
  	if (vScreenSteps[i]) {
	  m = i;
        }
  ...
  for (y = 0; y < 70; y++) {
    for (x = 0; x < 10; x++) {
      wattron(ser,COLOR_PAIR(11)); 
      r = 4 * vScreen[m][(x * 70 + y) * 3];
      g = 4 * vScreen[m][(x * 70 + y) * 3 + 1];
      b = 4 * vScreen[m][(x * 70 + y) * 3 + 2];
      if (r > 100 && b < 100 && g < 100) {
	      wattron(ser,COLOR_PAIR(8)); 
      }
      if (r < 100 && b < 100 && g > 100) {
	      wattron(ser,COLOR_PAIR(9)); 
      }
      if (r < 100 && b > 100 && g < 100) {
	      wattron(ser,COLOR_PAIR(10)); 
      }//      wmove(ser, 1, 1);
//      wprintw(ser, "loading pixel for step %d , pixel %d = %d,%d,%d        ", m, y * 10 + x, r, g, b);
//			sleep(1);
//      show(ser);
      if ((r + g + b) != 0) {
        wmove(ser, 5 + (70 - y), (x * 3) + 10);
        wprintw(ser, pix);
      }
    }
  }
...
#define PIXEL_ROW (MAX_PIXELS * 3 * 4 + 100)
//====================================
void readvScreen()
//====================================
{
  FILE *in;
  char *left,*right;  char *row,*line,*value,*p;
  char junk[10],*pch;
  int  step=0,res=0, pixel;
  row = (char *)malloc(PIXEL_ROW);
  line = (char *)malloc(PIXEL_ROW);
  value = (char *)malloc(PIXEL_ROW);
  
  in = fopen(fileServvScreen,"r");
  if(in == NULL)
    {
      showError("readvScreen: Unable to open file",-1);
      return;
    }
  else
    {
      while (fgets(row,PIXEL_ROW,in)!=NULL)
        {
          if(row[0] == '+')
            {
              //printf("vscreen %s\n",row);
              ...
              p = p+3;
              sscanf(row,"%s%d",junk,&step);
              vScreenSteps[step] = true;
//              printf("%d vscreenmod %s\n",step,p);
              //strcpy(data,p);
              pch = strtok(p,",");
              pixel = 0;
              while (pch != NULL)
                {
                  vScreen[step][pixel] = atoi(pch);
//                  printf("vscreen step=%d pixel=%d value=%d\n",step,pixel,vScreen[step][pixel]);
                  pixel++;
                  pch = strtok(NULL, ",");
                }
            }
